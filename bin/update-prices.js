...
function merge(csvData) {
	...
	const updatedPrices = rawData.reduce(
		(prices, rawRecord) => {
			if (!rawRecord.Date) {
				return prices;
			}
			...
			if (
				!existingPrices.some(
					(existingPrice) => existingPrice.date === rawRecord.Date,
				)
			) {
...
async function fetchPrices() {
	...
	if (!response.ok) {
		throw new Error(
			`There was a problem fetching the share price file: HTTP ${response.status} ${response.statusText}`,
		);
	}
